...
#---------------------------------------*********************------------------------------------------------#
    ...
    @classmethod
    def stopwRemove(cls,data):
        token_word = word_tokenize(data.lower())
    ...
    @staticmethod
    def sentFilter(old_data = None):
        new_data = SummDataClean.stopwRemove(old_data)
        data_list = new_data.split(". ")
        data_list_original = list(old_data.split(". "))
        s_lst = []
        s_lst.extend([re.sub('[^a-zA-Z]+', ' ', s) for s in data_list])

        return(data_list_original,s_lst) 
